...
class PCFG(object):
    ...
    def update_counts(self, parsed_sentence):
        ...
        for production in parsed_sentence.productions():
            ...
            self.lhs_counts[production.lhs()] += 1
    ...
    def compute_scores(self):
        ...
        #### YOUR CODE HERE ####
        for production in self.production_counts:
            log_numerator = np.log(self.production_counts[production])
            log_denominator = np.log(self.lhs_counts[production.lhs()])
            self.scored_productions[production] = log_numerator - log_denominator
