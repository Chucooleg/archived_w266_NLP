...
class CKYParser(object):
    ...
    def _apply_preterminal_rules(self, words, chart):
        ...
        for i, word in enumerate(words):
            ...
            word_results = self._grammar.lookup_rhs(word)
            if len(word_results)==0:
                return False
            chart[cell_key] = {lhs: ProbabilisticTree(lhs, [word], logprob=score)
                               for (lhs, score) in word_results}
            #print ([score for (lhs,score) in word_results])
                ...
                for B in B_lhs:
                    ...
                    for C in C_lhs:
                        ...
                        for A, weight_A_BC in self._grammar.lookup_rhs(B,C):
                            ...
                            ### Score calculation
                            score = chart[(i,split)][B].logprob() + chart[(split,j)][C].logprob() + weight_A_BC
                            
                            if score > chart[(i, j)][A].logprob():
                                #print ("Boo",score > chart[(i, j)][A].logprob())
                                chart[(i, j)][A] = ProbabilisticTree(A,(chart[(i, split)][B], chart[(split, j)][C]), logprob=score)
                                #print ("final logprob",chart[(i, j)][A].logprob())
