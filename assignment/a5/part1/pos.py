...
class HMM(object):
    ...
    def forward(self, sentence):
        ...
        HINT:  You can look at backward(...) to get a feel for how the various
               probability tables work.  However, as a warning, it's easier
               to start with the alpha equations in the Jupyter notebook and going
               from there to the code than starting with the code in backward()
               and trying to fix it.  There are just too many off-by-one
               differences.  (Though backward is a good piece of code to study
               in conjunction with the beta equations in the notebook to
               understand how that math can turn into code.)
        ...
        for i, w in enumerate(sentence):
            for t in self.tagset:
                if i==0:
                    alpha[(i,t)] = self.emission(t,w) + self.initial(t)
                else:
                    sum_terms = [ (self.transition(t_1,t) + alpha[(i-1,t_1)]) for t_1 in self.tagset]
    ...
    def build_viterbi_delta(self, sentence):
        ...
        for i,w in enumerate(sentence):
            for t in self.tagset:
                if i==0:
                    delta[(i,t)] = self.emission(t,w) + self.initial(t)
                else:
                    terms = [(self.transition(t_1, t) + delta[(i-1,t_1)], t_1)for t_1 in self.tagset]
                    max_term = sorted(terms, key=lambda x: x[0], reverse=True)[0]
