...
class AddKTrigramLM(BaseLM):
    ...
    def __init__(self, tokens):
        ...
        self.context_totals = {key: sum(val_dict.values()) for key, val_dict in self.counts.items()}
    ...
    def next_word_proba(self, word, seq):
        ...
        nom = self.counts.get(context,{}).get(word,0) + k
        denom = self.context_totals.get(context,0) + k*self.V
        return nom/denom
...
class KNTrigramLM(BaseLM):
    ...
    def __init__(self, tokens):
        ...
        self.context_totals = {context: sum(word_dict.values()) for context,word_dict in self.counts.items()}
        # Count the number of nonzero entries for each context.
        self.context_nnz = {context: len(list(word_dict.keys())) for context,word_dict in self.counts.items()}
        # Compute type fertilities, and the sum z_tf.
        self.type_fertility = {word : len(contexts) for word,contexts in self.type_contexts.items()}
    ...
    def kn_interp(self, word, context, delta, pw):
        ...
        # Hint: self.counts.get(...) and self.context_totals.get(...) may be
        # useful here. See note in dict_notes.md about how this works.
        if self.context_totals.get(context,0) == 0:
            return pw
        else:
            alpha = delta/self.context_totals.get(context,0)*self.context_nnz.get(context,0)
            discounted_mass = max(0,self.counts.get(context,{}).get(word,0)-delta)/self.context_totals.get(context,0)
            return discounted_mass + alpha*pw
        #### END(YOUR CODE) ####
