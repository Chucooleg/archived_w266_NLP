...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        with tf.name_scope("Embedding_Layer"):
            # C_ is our embedding matrix for look ups
        ...
        with tf.name_scope("Recurrent_Layer"):
            # Create a Dynamic RNN cell template
            ...
            self.o_, self.final_h_ = tf.nn.dynamic_rnn(cell=self.cell_, inputs=self.x_, sequence_length=self.ns_, initial_state=self.initial_h_, dtype=tf.float32 )
        ...
        with tf.name_scope("Output_Layer"):
            ...
            self.logits_ = tf.add(matmul3d(X=self.o_ , W=self.W_out_), self.b_out_, name="logits")


        # Loss computation (true loss, for prediction)
        with tf.name_scope("True_Cost"):
            self.per_example_loss_ = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.target_y_, logits=self.logits_, name="per_sample_loss")
    ...
    def BuildTrainGraph(self):
        ...
        with tf.name_scope("Training_Cost"):
            # argument weights expects shape [V, H]
            self.per_example_train_loss_ = tf.nn.sampled_softmax_loss(weights=tf.transpose(self.W_out_), biases=self.b_out_, inputs= tf.reshape(self.o_, [-1,self.H]), labels=tf.reshape(self.target_y_, [-1,1]), num_sampled=self.softmax_ns, num_classes=self.V, name="per_example_sampled_softmax_loss")
        ...
        with tf.name_scope("Training"): 
            ...
            self.gradients_, self.variables = zip(*self.optimizer_.compute_gradients(self.train_loss_))
            # clip the gradients, update them
            self.gradients_, _ = tf.clip_by_global_norm(self.gradients_, self.max_grad_norm_)
            # apply new gradients on optimizer
            self.train_step_ = self.optimizer_.apply_gradients(zip(self.gradients_, self.variables))


            # both compute gradient and apply to update model parameters
    ...
    def BuildSamplerGraph(self):
        ...
        with tf.name_scope("Prediction"):
            self.pred_random_ = tf.multinomial(logits=tf.reshape(self.logits_,[-1,self.V]), num_samples=1, name="pred_random")

            # reshape back to 3D
            self.pred_samples_ = tf.reshape(self.pred_random_,[self.batch_size_,self.max_time_,1])
