...
def affine_layer(hidden_dim, x):
    ...
    W = tf.get_variable("W", (x.shape[1],hidden_dim), initializer=tf.contrib.layers.xavier_initializer())
    b = tf.get_variable("b", (hidden_dim,), initializer=tf.zeros_initializer())
    # Return xW + b.
...
def fully_connected_layers(hidden_dims, x):
    ...
    for i in range(0,len(hidden_dims)):
        with tf.variable_scope("L"+str(i+1)):
...
def MakeLogits(x_ph, hidden_dims):
    ...
    '''
    # YOUR CODE HERE
    #print ("x_ph ",x_ph.get_shape())
    #fully_connected_layer = fully_connected_layers(hidden_dims, x_ph)
    #print ("fully_connected_layer", fully_connected_layer.get_shape())
    #final_affine_layer = affine_layer(1, fully_connected_layer)
    #print ("final_affine_layer", final_affine_layer.get_shape())
    #squeezed = tf.squeeze(final_affine_layer, axis=1)
    #print ("squeezed :", squeezed.get_shape())
...
def MakeLoss(logits, y_ph):
    ...
    Hint:  See tf.sigmoid_cross_entropy_with_logits (note the dimension it expects for logits,
