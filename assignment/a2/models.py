...
def embedding_layer(ids_, V, embed_dim, init_scale=0.001):
    ...
    W_embed_ = tf.get_variable("W_embed", (V, embed_dim),initializer = tf.random_uniform_initializer(minval=-init_scale, maxval=init_scale, dtype=tf.float32))
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    h0_ = tf.reduce_sum(xs_, 1)

    # Build a stack of fully-connected layers
    h_ = fully_connected_layers(h0_, hidden_dims, activation=tf.tanh, dropout_rate=dropout_rate, is_training=is_training)
